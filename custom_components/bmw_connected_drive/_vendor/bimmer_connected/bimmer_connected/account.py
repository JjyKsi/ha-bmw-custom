...
class MyBMWAccount:
    ...
    async def _init_vehicles(self) -> None:
        ...
        async with MyBMWClient(self.config) as client:
            headers = client.generate_default_header()
            params = {
                "apptimezone": 120,
                "appDateTime": int(fetched_at.timestamp() * 1000),
            }

            vehicle_list_response = await client.get(VEHICLES_URL, headers=headers, params=params)
            vehicle_entries = vehicle_list_response.json() or []

            for entry in vehicle_entries:
                vin = entry.get("vin") or entry.get("VIN")
                if not vin:
                    continue

                brand_raw = (entry.get("brand") or entry.get("Brand") or CarBrands.BMW.value).upper()

                profile: dict = {}
                profile_headers = dict(headers, **{"bmw-vin": vin})
                try:
                    profile_response = await client.get(VEHICLE_PROFILE_URL, headers=profile_headers)
                    if profile_response.status_code == 200:
                        profile = profile_response.json() or {}
                except Exception as ex:  # pragma: no cover - network/runtime issues bubble up as warnings
                    _LOGGER.warning("Unable to fetch profile for vehicle %s: %s", vin, ex)

                drive_train_raw = (
                    (profile.get("driveTrain") if isinstance(profile, dict) else None)
                    or entry.get("driveTrain")
                    or entry.get("appVehicleType")
                    or ""
                ).upper()
                valid_drivetrains = {dt.value for dt in DriveTrainType}
                drive_train = drive_train_raw if drive_train_raw in valid_drivetrains else DriveTrainType.UNKNOWN.value
                ...
                attributes = {
                    ...
                    or entry.get("Model")
                    or entry.get("appVehicleType"),
                    "appVehicleType": entry.get("appVehicleType"),
