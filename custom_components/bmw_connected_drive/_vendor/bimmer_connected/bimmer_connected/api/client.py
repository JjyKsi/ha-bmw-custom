...
from bimmer_connected.api.regions import get_app_version, get_region_config, get_server_url, get_user_agent
...
class MyBMWClient(httpx.AsyncClient):
    ...
    def generate_default_header(self, brand: Optional[CarBrands] = None) -> Dict[str, str]:
        ...
        region_config = self.config.authentication.region_config
        brand_value = (brand or CarBrands.BMW).value
        evcc_user_agent = X_USER_AGENT.format(
            build_string="SP1A.210812.016.C1",
            brand=brand_value,
            app_version="99.0.0(99999)",
            region=region_config.x_user_agent_region,
        )
        return {
