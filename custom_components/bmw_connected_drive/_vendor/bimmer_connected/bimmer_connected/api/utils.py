...
_X_USER_AGENT_BUILDSTRING: Optional[str] = None
...
def get_x_user_agent_buildstring():
    ...
    global _X_USER_AGENT_BUILDSTRING
    if _X_USER_AGENT_BUILDSTRING is not None:
        return _X_USER_AGENT_BUILDSTRING
    ...
    base = f"AP2A.{numeric}.{build_num}"

    # introduce a per-process random suffix to soften request fingerprints
    rand_suffix = random.SystemRandom().randint(0, 999)
    _X_USER_AGENT_BUILDSTRING = f"{base}-{rand_suffix:03d}"
