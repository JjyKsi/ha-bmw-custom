...
class MyBMWVehicle:
    ...
    async def get_vehicle_state(self) -> None:
        ...
        async with MyBMWClient(self.account.config) as client:
            ...
            state_response = await client.get(VEHICLE_STATE_URL, params=params, headers=headers)
            vehicle_state: Dict = state_response.json() or {}

            # Ensure expected top-level containers are present so downstream parsing works with EVCC payloads
            vehicle_state.setdefault(ATTR_STATE, vehicle_state.get("state", {}))
            vehicle_state[ATTR_STATE] = vehicle_state.get(ATTR_STATE) or {}

            self.update_state([vehicle_state], fetched_at)
    ...
    def drive_train(self) -> DriveTrainType:
        ...
        value = self.data[ATTR_ATTRIBUTES].get("driveTrain", DriveTrainType.UNKNOWN.value)
        try:
            return DriveTrainType(value)
        except ValueError:
